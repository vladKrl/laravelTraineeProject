        ...
        <div className="divide-y overflow-hidden">
            ...
            <div className="overflow-y-auto h-full">
                {conversations.map(conv => (
                    <Link key={conv.id} href={`/conversations/${conv.id}`} className={`block p-4 ${(id && Number(id) === Number(conv.id) ) ? 'bg-gray-200 hover:bg-gray-300' : 'hover:bg-gray-200'} transition`}>
                        <div className={"flex justify-center items-center gap-3"}>
                            <div className="relative">
                                {conv.interlocutor.profile.avatar ? (
                                    <img alt="" src={conv.interlocutor?.profile.avatar} className="w-12 h-12 rounded-full object-cover" />
                                ) : (
                            ...
                            <div className="flex-grow min-w-0">
                                <div className="flex justify-between items-baseline">
                                    <h4 className="font-semibold truncate text-sm">{conv.product.title}</h4>
                                    <span className="text-[10px] text-gray-600">
                                        {new Date(conv.last_message?.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                    </span>
                                </div>
                                <p className="text-xs text-gray-500 truncate mt-1">
                                    {conv.last_message?.user_id === currentUserId ? 'You: ' : ''}
